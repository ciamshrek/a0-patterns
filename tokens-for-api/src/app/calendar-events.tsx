...
import { getGoogleCalEvents, type IResponse } from "@/app/actions/getGoogleCalEvents"
...
function SubmitButton() {
  ...
  return (
    <Button type="submit" size="lg" className="w-full" disabled={pending}>
      {pending ? "Fetching..." : "Fetch Calendar Events"}
    </Button>
  )
}
    ...
    <div className="flex items-center justify-center min-h-screen bg-background p-4">
      <Card className="w-full max-w-md mx-auto">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">Tokens for APIs Demo</CardTitle>
          <CardDescription className="pt-2">
            Access your Google Calendar events using our secure token management system.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          {state?.status === "success" ? (
                    ...
                    state.events.map((event, index) => (
                      <div key={event.id || index} className="border rounded-lg p-3">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {formatEventDate(event.start?.dateTime || event.start?.date || undefined)} -
                          {formatEventDate(event.end?.dateTime || event.end?.date || undefined)}
                        </p>
                      </div>
                    ))}
                </div>
            ...
            <div className="text-center space-y-4 py-4">
              ...
              {'error_code' in state && state.error_code === 'refresh_token_not_found' && (
                <div className="flex flex-col items-center gap-2">
                  <AlertCircle className="h-10 w-10 text-amber-500" />
                  <p>You need to grant permission to access your Google Calendar.</p>
                </div>
              )}
